...
namespace XAPI
{
    public class XAPIMessenger : MonoBehaviour
    {
        ...
        /// <param name="callback"></param>
        /// <returns></returns>
        public static IEnumerator SendStatementRoutine(Statement statement, Action<Statement, string, UnityWebRequest> callback = null)
        {
            // Serialize this thing then convert into a utf-8 byte array
            var statementString = JsonConvert.SerializeObject(statement);
            var request = XAPIMessenger.BuildPostRequest(XAPIMessenger.Config.StatementEndpoint, statementString);
            ...
            if (callback != null)
            {
                ...
                try 
                {
                    statementKey = JsonConvert.DeserializeObject<string[]>(request.downloadHandler.text)[0];
                }
                catch (Exception ex)
                {
                ...
                callback(statement, statementKey, request);
            }
        }
        #endregion
